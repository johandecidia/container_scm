...
celery: ## Start Celery and celery beat
	@uv run celery -A container_scm worker -l INFO --beat --pool=solo
...
migrate: ## Run DB migrations in the container
	...
	@until docker compose exec db pg_isready -d container_scm -U postgres >/dev/null 2>&1; do echo "Database not ready, waiting..."; sleep 2; done
...
dbshell: ## Get a Database shell
	@docker compose exec db psql -U postgres container_scm
