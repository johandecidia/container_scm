...
Create a database named `container_scm`.

```
createdb container_scm
```
...
```bash
celery -A container_scm worker -l INFO --pool=solo
```

Or with celery beat (for scheduled tasks):

```bash
celery -A container_scm worker -l INFO -B --pool=solo
```
